...
class Fetch
{
  private:
    ...
    std::unique_ptr<Cemu> m_cemu = std::make_unique<Cemu>();
  ...
  public:
    std::unique_ptr<Platform> get_platform(ns_enum::Platform platform) const
    {
      switch (platform)
      {
        ...
        case ns_enum::Platform::CEMU      : return std::make_unique<Platform>(*m_cemu);
      } // switch
...
inline std::expected<Fetch, std::string> read_impl(fs::path const& path_file_db)
{
  return ns_db::from_file<std::expected<Fetch, std::string>>(path_file_db, [&](auto&& db) -> std::expected<Fetch, std::string>
  {
    ...
    // Melonds (tolerate absence: manifests predating melonds support lack this key)
    ...
    // Cemu (tolerate absence: manifests predating cemu support lack this key)
    if (auto value = db.template value<std::string>("cemu", "layer"))
    {
      fetch.m_cemu->m_url_layer["default"] = *value;
    } // if
    // Wine
